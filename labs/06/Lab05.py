...
def make_function_call(name, *args):
    return {TYPE: "func", NAME: name, ARGS: list(args)}
...
def make_neg(sentence):
    new_formula = deepcopy(sentence)
    return {TYPE: "formula", NAME: "neg", ARGS: [new_formula]}
...
def make_and(sentence1, sentence2, *others):
    ...
    return {TYPE: "formula", NAME: "and", ARGS: args + list(others)}
...
def make_or(sentence1, sentence2, *others):
    return {TYPE: "formula", NAME: "or", ARGS: [sentence1, sentence2] + list(others)}
...
def replace_args(formula, new_args):
    if len(new_args) != len(formula[ARGS]):
        return None
    
    new_form = deepcopy(formula)
    new_form[ARGS] = new_args
    return new_form
...
def is_sentence(f):
    return f is not None and (f[TYPE] in ["atom", "formula"])
...
def get_args(f):
    if f is not None:
        if f[TYPE] in ["sentence", "func", "atom", "formula"]:
            return f[ARGS]
    return None
...
def unify(f1, f2, subst = None):
    ...
    while len(S) != 0:
        ...
        if s != t:
            ...
            if has_args(s) and has_args(t) and len(get_args(s)) == len(get_args(t)) and not ok:
                if get_head(s) == get_head(t):
                    ok = True
                    map(lambda x: S.append(x), zip(get_args(s), get_args(t)))
                else:
                    return False
